...
if(isset($_POST["btnreset"]))
{
	...
	{
		$sela="select * from tbl_admin where admin_mail='".$_SESSION["femail"]."'";
		$rowa=$con->query($sela);
		$sell="select * from tbl_busowner where owner_mail='".$_SESSION["femail"]."'";
		$rowl=$con->query($sell);
		...
		{
			$up="update tbl_admin set admin_pass='".$_POST["txtnew"]."' where admin_mail='".$_SESSION["femail"]."'";
			$con->query($up);
		}
		else if($datal=$rowl->fetch_assoc())
		{
			$up="update tbl_busowner set owner_pass='".$_POST["txtnew"]."' where owner_mail='".$_SESSION["femail"]."'";
			$con->query($up);
		}
		else if($datau=$rowu->fetch_assoc())
		{
			$up="update tbl_user set user_pass='".$_POST["txtnew"]."' where user_email='".$_SESSION["femail"]."'";
			$con->query($up);
		}
		else
		{
  ...
  <table width="407" border="1" cellpadding="10">
   <h2> Reset Password</h2>
    <tr>
     ...
     <i class="fa fa-eye "id="togglePassword" cursor: pointer;"></i>
...
<a href="..\Guest\Login.php">Go Back To Login</a>
</div>
