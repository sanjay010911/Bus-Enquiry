...
<title>Bus Enquiry:Rejected Buses</title>
</head>
<?php
ob_start();
include("head.php");
...
{
		$did=$_GET["did"];
		$sql="update tbl_busdetails set bus_status='1' where bus_id='".$did."'";
		if($con->query($sql))
		{
			?>
            <script>
			alert("Accepted");
			location.href="BusRejected.php";
            </script>
            <?php	
   		}
		else
...
<form id="form1" name="form1" method="post" action="">
  <table width="415" border="1" cellpadding="10">
   ...
   $selQry="select * from tbl_busdetails b inner join tbl_bustype t on b.bustype_id=t.bustype_id inner join tbl_route r on 
	         b.route_id=r.route_id inner join tbl_busowner o on b.owner_id=o.owner_id where bus_status='2'";
   ...
   while($result=$row->fetch_assoc())
	{
      ...
      <td><img src="../Assets/Files/Busphoto/<?php echo $result["bus_photo"];?>"></td>
       <td><a href="BusRejected.php?did=<?php echo $result["bus_id"];?>">Accept</a><br/></td>
       <?php
	$i++;
	}
