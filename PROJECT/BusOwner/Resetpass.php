...
include("Head.php");
	...
	if(isset($_POST["btnsave"]))
	{
		...
		$selq="select * from tbl_busowner where owner_pass='".$_POST["txt_cur"]."' and owner_id='".$_SESSION["lgid"]."'";
		if($con->query($selq))
		{
				if($_POST["txt_new"]==$_POST["txt_con"])
				{
					$up="update tbl_busowner set owner_pass='".$_POST["txt_new"]."' where owner_id='".$_SESSION["lgid"]."'";
					if($con->query($up))
					{
						?>
...
<form id="form1" name="form1" method="post" action="">
  <table width="450" height="272"  border="1" align="center" cellpadding="10">
    <tr>
      <td width="145">Current Password</td>
      <td width="216"><label for="txt_cur"></label>
      <input type="text" name="txt_cur" id="txt_cur" required="required" autocomplete="off" /></td>
    </tr>
    <tr>
      <td>New Password</td>
      <td><label for="txt_new"></label>
      <input type="text" name="txt_new" id="txt_new" required="required" autocomplete="off" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters" required/></td>
    </tr>
    <tr>
